...
# given that maxSum[i-2] is generated by the sub array tht goes from 0 to i-2 index. 
# therefore, we can safelly add array[i] to compose the new maxSum[i] if it results in sum bigger that maxSum[i-1]
